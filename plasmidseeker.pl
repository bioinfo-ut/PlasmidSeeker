...
my $glistquery = "scripts/glistquery";
my $glistcompare = "scripts/glistcompare";
my $gdistribution = "scripts/gdistribution";
my $rtest = "scripts/testfunktsioon_ver3_uus.R";
...
my $prc_limit = 65; # Percent of plasmid unique k-mers that have to be found from sample
...
sub printHelp {
	...
	-p\t Minimum percentage of unique k-mers that have to be found (default 65)\n
	-h\t Print this help
...
if(!$ponly) {
	...
	if($bacterial_cov<=3) { die "Bacteria median coverage is too low (3x or less). Larger dataset is needed!"; }
}
...
foreach(@files) {
	$count++;
	$cmd_cmp .= "$glistcompare $wgs_list $dir_location/$_ -i -r first --count_only --print_operation &";
...
foreach(keys %highcov) {
	...
	$results{$_}{'Plasmid_total'} = $highcov{$_}{'Total_unique'};
	...
	my $zero_kmers = $results{$_}{'Plasmid_total'} - $results{$_}{'Found_sample'}; # Kmers not found
	...
	my $cut_coverage = sprintf("%.2f",$cutlist{"tmp_final_$word\_0_diff1.list"}{'P_unique'}/$results{$_}{'Plasmid_total'}*100);
	...
	$results{$_}{'Covered'} = int(($results{$_}{'Percent'}/100)*$results{$_}{'Plasmid_total'});
	...
	#print "$results{$_}{'Percent'}\t$_\t$results{$_}{'Cov'}\t$results{$_}{'Median'}\t$plasmid_id\n";
}
...
foreach(keys %results) {
	shift @samples;
	#print "\n\tComparing: $_\n";
	if(exists $done{$_}) { next; } # Skips plasmids that are already in cluster
	
	foreach my $a (@samples) { #Search all results
		if(exists $done{$a}) { next; }
		
		my $cmd_isect = "$glistcompare $_ $a -i -r first --count_only";
		my @isect = qx/$cmd_isect/;
		my $shared = (split(/\t/,$isect[0]))[1];
		
		#print "$_ - $results{$_}{'Plasmid_total'} | $a - $results{$a}{'Plasmid_total'} | $shared\n";
		my $smaller = $results{$a}{'Plasmid_total'};
		if($results{$a}{'Plasmid_total'}>$results{$_}{'Plasmid_total'}) { $smaller = $results{$_}{'Plasmid_total'}; }
		if($shared/$smaller*100>$cluster_prc) {
			$cluster{$count}{$a} = $results{$a}{'Covered'};
			if(!exists $cluster{$count}{$_}) { $cluster{$count}{$_} = $results{$_}{'Covered'}; }
			$done{$a} = 1;
			$done{$_} = 1;
		}
	}
...
print $fh "# Unique k-mers found\tTotal unique kmers\t%Kmers found\tCopy number\tP-value\tPlasmid ID\tCoverage\tList file\n# MinP $prc_limit\n";
if(!$ponly) {
...
foreach(keys %cluster) {
	print $fh "\tPLASMID CLUSTER $count\n";
	$count++;
	my %single_cluster = %{$cluster{$_}};
	foreach my $name (sort { $single_cluster{$b} <=> $single_cluster{$a} } keys %single_cluster) {
		print $fh "$results{$name}{'Covered'}\t$results{$name}{'Plasmid_total'}\t$results{$name}{'Percent'}%\t$results{$name}{'Cov_real'}\t$results{$name}{'Pval'}\t$results{$name}{'PlasmidID'}\t$results{$name}{'Median'}\t$name\n";
	}
}

print $fh "\tSINGLE PLASMIDS\n";
#Single plasmids
foreach(keys %results) {
	if(!exists $done{$_}) {
		print $fh "$results{$_}{'Covered'}\t$results{$_}{'Plasmid_total'}\t$results{$_}{'Percent'}%\t$results{$_}{'Cov_real'}\t$results{$_}{'Pval'}\t$results{$_}{'PlasmidID'}\t$results{$_}{'Median'}\t$_\n";
	}
}
